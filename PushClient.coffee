...
class PushClient
  ...
  receiveMessage: (message) ->
    ...
    if parsedMessage.type == 'tickle'
      if push = @getLatestPush()
        @sendPush push

  receiveLatestPush: (push) ->
    @sendPush push
  
  getLatestPush: (test = false) ->
    ...
    Request requestOptions, (err, res, body) =>
      ...
      if not err and res.statusCode is 200
        ...
        parsed = JSON.parse(body).pushes[0]
        @latestPush = new Pushed parsed
        @latestPush.raw = parsed
        @receiveLatestPush @latestPush unless test
      else
